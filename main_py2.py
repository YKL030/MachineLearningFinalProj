...
for epoch in range(NUM_EPOCHS):
    ...
    for i, data in enumerate(valid_loader):
        ...
        loss_Y = criterion(outputs,Y_test)
        testing_loss += loss_Y.item()
        outputs = binary(outputs)
        ...
        for index in range(len(outputs)):
            if outputs[index]==Y_test[index]:
                testing_correct += 1
    print("Running_Loss is:{:.4f},testing_loss is:{:.4f},Train Acurracy is:{:.4f}%,Test Accuracy is:{:.4f}%".format(running_loss/(len(train_dataset)),
                                                                                                                    testing_loss/len(valid_dataset),
    ...
    if 100*testing_correct/len(valid_dataset)>64:
        with open("E:/progaramming/python_files/archivetest1.csv","w", newline = '') as csvfile: 
             ...
             for i, data in enumerate(test_loader):
                 ...
                 torch.save(model.state_dict(),"DenseNet_parameter5.pkl")  
            
                 
    torch.save(model.state_dict(),"DenseNet_parameter4.pkl")    
    # evaluate
